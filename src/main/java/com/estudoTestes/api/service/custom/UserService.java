...
public class UserService extends CRUDService<User, Integer, UserDTO> {
    ...
    @Override
    protected void updateEntityData(UserDTO userDTO, User user) {
    ...
    @Override
    protected void validateNewEntity(UserDTO userDTO, User user) {
        boolean userAlreadyExists = userRepository.findByEmail(userDTO.email()).isPresent();
        if (userAlreadyExists) {
            throw new RuntimeException("Usuário já existe com email: " + userDTO.email());
        }
    }
}